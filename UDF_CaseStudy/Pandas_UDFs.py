...
#Importing Data from Web Api with Pandas/DataReader/yfinance    
...
def getdata(symbols, startdate, enddate):
    ...
    df = yf.download(tickers = symbols, start = startdate, end = enddate, progress = False).Close
    return df
...
def financial_data(symbols, startdate, enddate, 
    ...
    #prices = data.DataReader(name = symbols, data_source = "yahoo", 
                         #start = startdate, end = enddate).Close
    prices = yf.download(tickers = symbols, start = startdate, end = enddate, progress = False).Close
